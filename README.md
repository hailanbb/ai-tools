...
| **last30days-skill-cn** | 专门面向中国 8 大主流平台（小红书、知乎、抖音、微信、B站、百度、微博、头条）最近 30 天内容深度研究与热度加权分析，生成 Swiss 风格离线 HTML 研报 | 针对国内主流社交/视频平台进行近期热点研究、品牌舆情监控及生成可视化 HTML 报告时 | [👉 详细配置与使用指南](tools/web-retrieval/last30days-skill-cn/README.md) |
...
| **aham-voice** | 本地优先的离线录音转写与会议纪要工具（macOS），集成 FunASR 离线转写、CAM++ 说话人分离与情绪感知 | 对保密会议录音进行离线音频转写、说话人声纹识别并调用 LLM 生成结构化会议纪要时 | [👉 详细配置与使用指南](tools/office-creative/aham-voice/README.md) |
...
| **awesome-llm-apps** | 收录 100+ 端到端开源 AI Agent、Agent Skills 和 RAG 应用实战例程与案例库 | 需要快速构建、测试或参考各种场景下的生产级 AI Agent 与 RAG 应用时 | [👉 详细配置与使用指南](tools/productivity-developer/awesome-llm-apps/README.md) |
...
| **ai-website-cloner-template** | 面向 AI Coding Agent 的 Next.js 网页逆向与像素级复刻模板，内置 `/clone-website` 技能，自动提取设计 Token、多组件并行重构 | 当需要通过 AI Agent 对现存网站进行 DOM/样式逆向分析、提取设计系统并快速复刻为现代 Next.js 项目时 | [👉 详细配置与使用指南](tools/productivity-developer/ai-website-cloner-template/README.md) |
