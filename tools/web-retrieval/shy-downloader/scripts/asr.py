...
def transcribe_with_whisper(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(dir=str(output_dir)) as temp_dir:
        ...
        # Restore punctuation from segments (Whisper v2 returns segments with timestamps)
        segments = data.get("segments", [])
        if segments:
            punctuated = _restore_punctuation_from_segments(segments)
            transcript_path.write_text(punctuated + "\n", encoding="utf-8")
            data["punctuated_text"] = punctuated
        else:
            # Fallback: apply regex-based punctuation restoration
            punctuated = _restore_chinese_punctuation(transcript)
            transcript_path.write_text(punctuated + "\n", encoding="utf-8")
...
def transcribe_with_siliconflow(
    ...
    *,
    ...
    # Restore punctuation from words array (SenseVoiceSmall returns word-level timestamps)
    words = data.get("words", [])
    if words:
        punctuated = _restore_punctuation_from_words(words)
        transcript_path.write_text(punctuated + "\n", encoding="utf-8")
        data["punctuated_text"] = punctuated
    else:
        # Fallback: apply regex-based punctuation restoration
...
def default_prompt_for(language: str) -> str | None:
    if language and language.lower() in {"chinese", "zh", "mandarin"}:
        return "请使用简体中文转写，不要使用繁体中文。保留专有名词、英文缩写和产品名称。请在转写中正确添加标点符号，包括句号、逗号、问号、感叹号等。"
    return None


def _restore_punctuation_from_words(words: list[dict]) -> str:
    """Restore punctuation from SenseVoiceSmall word-level JSON.
    
    Strategy: use time gaps between words to infer sentence boundaries.
    - Gap > 0.8s → period (。)
    - Gap > 0.3s → comma (，)
    - Existing punctuation in word text is preserved
    
    Returns lines separated by newlines, each line = one sentence.
    """
    if not words:
        return ""
    
    # Clean up words — remove empty and pure-whitespace entries
    cleaned = []
    for w in words:
        t = w.get("text", "").strip()
        if not t:
    ...
    sentences = []
    current = []
    last_end = float(cleaned[0].get("start", 0))
    ...
    for i, w in enumerate(cleaned):
        ...
        # Check gap to next word (if not last word)
        if i < len(cleaned) - 1:
            next_start = float(cleaned[i + 1].get("start", end))
            gap = next_start - end
...
def _restore_punctuation_from_segments(segments: list[dict]) -> str:
    """Restore punctuation from Whisper segments.
    
    Strategy: use time gaps between segments to infer sentence boundaries.
    - Gap > 0.8s → period (。)
    - Gap > 0.3s → comma (，)
    - Existing punctuation in segment text is preserved
    ...
    for i, seg in enumerate(segments):
        ...
        if i < len(segments) - 1:
            next_start = float(segments[i + 1].get("start", end_time))
    ...
    # Post-process: unify English punctuation to Chinese
...
def _restore_chinese_punctuation(text: str) -> str:
    """Regex-based punctuation restoration for Chinese text (fallback).
    
    Conservative heuristic:
    - Split at every ~40 Chinese characters with a period
    - Preserve English words and numbers intact  
    - Insert commas at natural breaks (~15-20 chars)
    """
    if not text:
        return ""
    ...
    for para in paragraphs:
        ...
        # Tokenize into: Chinese blocks, English/number blocks, and other non-Chinese
        tokens = re.findall(r'[\u4e00-\u9fff]+|[a-zA-Z0-9_.\-]+|[^\u4e00-\u9fff]+', para)
        ...
        for token in tokens:
            cn_chars_in_token = len(re.findall(r'[\u4e00-\u9fff]', token))
            
            if cn_chars_in_token > 0:
                # This token contains Chinese chars
                
                # If adding this would exceed 40 Chinese chars, flush first
                if chinese_count + cn_chars_in_token > 40:
                    if current_chunk:
                        sent = ''.join(current_chunk)
                        if len(sent) > 15:
                            # Find a good split point near the middle
                            mid = len(sent) // 2
                            while mid < len(sent) and not re.match(r'[\u4e00-\u9fff]', sent[mid]):
                                mid += 1
                            first_half = sent[:mid]
                            second_half = sent[mid:]
                            if not first_half.endswith(('。', '！', '？')):
                                first_half += '，'
                            sentences.append(first_half)
                            current_chunk = [second_half]
                            chinese_count = len(re.findall(r'[\u4e00-\u9fff]', second_half))
                            continue
                        else:
                            sent = ''.join(current_chunk)
                            if not sent.endswith(('。', '！', '？')):
                            ...
                            sentences.append(sent)
                    current_chunk = [token]
                    chinese_count = cn_chars_in_token
                else:
                    current_chunk.append(token)
                    chinese_count += cn_chars_in_token
            else:
                # Non-Chinese token (English word, number, URL, etc.)
                current_chunk.append(token)
