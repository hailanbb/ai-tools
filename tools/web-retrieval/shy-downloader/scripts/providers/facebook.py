"""Facebook provider backed by yt-dlp."""
...
def supports(url: str) -> bool:
    ...
    return any(d in host for d in ("facebook.com", "fb.com", "fb.watch"))

def fetch(url: str, output_root: Path, *, metadata_only: bool = False, **options) -> dict:
    yt = _require_ytdlp()
    commands = [[yt, "--no-playlist", "--dump-single-json", url], [yt, "--cookies-from-browser", "chrome", "--no-playlist", "--dump-single-json", url]]
    metadata = json.loads(_run_first(commands))
    item_id = str(metadata.get("id", "unknown"))
    folder = output_root / f"facebook-{item_id}"
...
def _run_first(commands, timeout=240) -> str:
    errors = []
    for cmd in commands:
        r = subprocess.run(cmd, capture_output=True, text=True, timeout=timeout)
        if r.returncode == 0: return r.stdout
        errors.append(r.stderr.strip() or r.stdout.strip()[:200])
    raise RuntimeError("yt-dlp failed: " + " | ".join(errors))

def _download(yt, url, folder, name):
    out = folder / name
    commands = [[yt, "--no-playlist", "-f", "bv*+ba/b", "--merge-output-format", "mp4", "-o", str(out), url], [yt, "--cookies-from-browser", "chrome", "--no-playlist", "-f", "bv*+ba/b", "--merge-output-format", "mp4", "-o", str(out), url]]
    _run_first(commands, timeout=900)
