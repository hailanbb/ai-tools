...
The script supports two directory layouts:
    ...
    Workspace layout (from skill-creator-cn iterations):
    <benchmark_dir>/
    └── eval-N/
        ├── with_skill/
        │   ├── run-1/grading.json
...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    if runs_dir.exists():
        ...
        print(f"在 {benchmark_dir} 或 {benchmark_dir / 'runs'} 未找到 eval 目录")
        return {}
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                ...
                for exp in raw_expectations:
                    if "text" not in exp or "passed" not in exp:
                        print(f"警告：{grading_file} 中的 expectation 缺少必需字段 (text, passed, evidence)：{exp}")
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines = [
        f"# Skill 量化对比：{metadata['skill_name']}",
        "",
        f"**模型**：{metadata['executor_model']}",
        f"**日期**：{metadata['timestamp']}",
        f"**评测集**：{', '.join(map(str, metadata['evals_run']))}（每配置各 {metadata['runs_per_configuration']} 次）",
        "",
        "## 摘要",
        "",
    ...
    lines.append(f"| Token 数 | {a_tokens.get('mean', 0):.0f} ± {a_tokens.get('stddev', 0):.0f} | {b_tokens.get('mean', 0):.0f} ± {b_tokens.get('stddev', 0):.0f} | {delta.get('tokens', '—')} |")
