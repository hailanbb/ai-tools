...
def run_loop(
    ...
    if holdout > 0:
        ...
        if verbose:
            print(f"拆分：{len(train_set)} 训练 / {len(test_set)} 测试（holdout={holdout}）", file=sys.stderr)
    else:
        train_set = eval_set
        test_set = []
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if verbose:
            def print_eval_stats(label, results, elapsed):
                ...
                for r in results:
                    ...
                    print(f"  [{status}] 命中率={rate_str} 期望={r['should_trigger']}：{r['query'][:60]}", file=sys.stderr)
        ...
        if train_summary["failed"] == 0:
            ...
            if verbose:
                print(f"\n第 {iteration} 轮所有训练查询全部通过！", file=sys.stderr)
            break
        ...
        if verbose:
            print(f"提议（{improve_elapsed:.1f}s）：{new_description}", file=sys.stderr)
